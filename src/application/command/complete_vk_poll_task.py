...
class CompleteVKPollTaskCommand:
    event_id: str | None
    vk_user_id: int
    voted_poll_external_ids: tuple[str, ...]


class CompleteVKPollTaskHandler(
    Interactor[CompleteVKPollTaskCommand, TaskCompletionResult],
):
    """Засчитывает голосование в VK-опросе, если по нему есть активное задание."""
    ...
    async def __call__(
        ...
        task = await self.task_repository.get_active_poll_task_by_external_ids(
            external_ids=command_data.voted_poll_external_ids,
        )
        if task is None:
