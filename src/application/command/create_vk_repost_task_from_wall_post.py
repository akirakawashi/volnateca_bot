...
REPOST_TASK_MARKER = "#volnateca_repost_task"
REPOST_TASK_POINTS_PATTERN = re.compile(r"#volnateca_points_(?P<points>\d+)", re.IGNORECASE)
REPOST_TASK_WEEK_PATTERN = re.compile(r"#volnateca_week_(?P<week>\d+)", re.IGNORECASE)
DEFAULT_REPOST_TASK_POINTS = 20
MAX_TASK_DESCRIPTION_LENGTH = 500


@dataclass(slots=True, frozen=True, kw_only=True)
class CreateVKRepostTaskFromWallPostCommand:
    event_id: str | None
    group_id: int
    post: VKWallPostDTO
    text: str
...
@dataclass(slots=True, frozen=True, kw_only=True)
class ParsedVKRepostTaskMarker:
    points: int
    week_number: int | None


class CreateVKRepostTaskFromWallPostHandler(
    Interactor[CreateVKRepostTaskFromWallPostCommand, VKRepostTaskCreationDTO],
):
    def __init__(
        self,
    ...
    async def __call__(
        ...
        if canonical_post is None:
            return VKRepostTaskCreationDTO(
                status=VKRepostTaskCreationStatus.WRONG_WALL_OWNER,
        ...
        if parsed_marker is None:
            return VKRepostTaskCreationDTO(
                status=VKRepostTaskCreationStatus.IGNORED,
                event_id=command_data.event_id,
                external_id=canonical_post.external_id,
                reason="repost_task_marker_not_found",
            )
        if parsed_marker.points <= 0:
            return VKRepostTaskCreationDTO(
                status=VKRepostTaskCreationStatus.INVALID_MARKER,
                event_id=command_data.event_id,
                external_id=canonical_post.external_id,
        ...
        result = await self.repository.create_repost_task_if_not_exists(
        ...
        logger.info(
            "TEMP VK repost task creation handled: "
    ...
    @staticmethod
    def _parse_marker(text: str) -> ParsedVKRepostTaskMarker | None:
        if REPOST_TASK_MARKER not in text.lower():
            return None

        points = DEFAULT_REPOST_TASK_POINTS
        points_match = REPOST_TASK_POINTS_PATTERN.search(text)
        if points_match is not None:
            points = int(points_match.group("points"))

        week_number = None
        week_match = REPOST_TASK_WEEK_PATTERN.search(text)
        if week_match is not None:
            week_number = int(week_match.group("week"))

        return ParsedVKRepostTaskMarker(points=points, week_number=week_number)

    @staticmethod
    def _build_task_code(post: VKWallPostDTO) -> str:
        return f"vk_repost_wall_{abs(post.owner_id)}_{post.post_id}"

    @staticmethod
    def _build_task_name(parsed_marker: ParsedVKRepostTaskMarker) -> str:
        if parsed_marker.points >= 60:
