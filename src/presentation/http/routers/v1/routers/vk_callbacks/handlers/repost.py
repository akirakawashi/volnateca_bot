...
from presentation.http.routers.v1.routers.vk_callbacks.handlers.reward_delivery import (
    send_like_reward_message,
)
...
async def handle_repost_callback(
    ...
    if result.status == TaskCompletionResultStatus.COMPLETED:
        await send_repost_reward_message(
            data=data,
            result=result,
    ...
    if like_result.status == TaskCompletionResultStatus.COMPLETED:
        await send_like_reward_message(
            data=data,
            result=like_result,
            message_client=message_client,
...
async def send_repost_reward_message(
    *,
    data: VKCallbackPayload,
