...
def build_quiz_offer_message(*, task_name: str, points: int) -> VKMessageText:
    return build_template_message("quiz_offer", task_name=task_name, points=points)
...
def build_quiz_answer_result_message(
    *,
    ...
    if is_correct:
        return build_template_message("quiz_answer_correct")
    correct_hint = f"\nПравильный ответ: {correct_option_text}" if correct_option_text else ""
    return build_template_message("quiz_answer_incorrect", correct_hint=correct_hint)


def build_quiz_unavailable_message() -> VKMessageText:
    return build_template_message("quiz_unavailable")
...
def build_quiz_failed_message() -> VKMessageText:
    return build_template_message("quiz_failed")
